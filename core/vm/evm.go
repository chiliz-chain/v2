...
func (a hookStateDBAdapter) SetCode(addr common.Address, code []byte) {
	a.inner.SetCode(addr, code, tracing.CodeChangeSystemContractUpgrade)
}
...
func (evm *EVM) CreateWithAddress(caller common.Address, code []byte, gas uint64, value *big.Int, contractAddr common.Address) (ret []byte, leftOverGas uint64, err error) {
	ret, _, leftOverGas, err = evm.create(caller, code, gas, uint256.MustFromBig(value), contractAddr, STOP)
	return
}
