...
func (bc *BlobConfig) blobBaseFee(excessBlobGas uint64) *big.Int {
	return fakeExponential(MinBlobGasprice(bc.config), new(big.Int).SetUint64(excessBlobGas), new(big.Int).SetUint64(bc.UpdateFraction))
}
...
func MinBlobGasprice(config *params.ChainConfig) *big.Int {
	if config != nil && config.Parlia != nil {
		return big.NewInt(params.BlobTxMinBlobGaspriceForBSC)
	}
