...
func newTestParliaHandlerAfterCancun(t *testing.T, config *params.ChainConfig, mode ethconfig.SyncMode, preCancunBlks, postCancunBlks uint64) *testHandler {
	...
	gspec := &core.Genesis{
		Config: config,
		// Fund generously: this is a Parlia config, where the base fee is
		// InitialBaseFeeForBSC (2500 gwei) and the blob fee BlobTxMinBlobGaspriceForBSC
		// (150 gwei). The sender pays these for a transaction in every generated
		// block, so it needs enough balance to cover all of them.
		Alloc: types.GenesisAlloc{testAddr: {Balance: new(big.Int).Mul(big.NewInt(params.Ether), big.NewInt(1_000_000))}},
	}
	engine := &mockParlia{}
